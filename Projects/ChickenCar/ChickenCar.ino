/*arduino:avr:mega
	...
	Version 2022-06-24 
		adds RF Version requests, and adjusted music and time intervals
		fixes some warnings
  ...
  ** The Chicken car also has an RF remote.  Covered here is only the chicken car itself.
  ...
  Description:
	...
	What is auto chicken ?
		Move forward every 6 minutes (AUTO_CHICKEN_MVMNT_INTERVAL), 4 times (AUTO_CHICKEN_MVMNT_COUNT) , then back 4 times every 6 minutes
...
#define AUTO_CHICKEN_MVMNT_COUNT 4 // count of times to move forward and backward
#define AUTO_CHICKEN_MVMNT_INTERVAL (60000 * 6)	// 6 min. Time in milliseconds between movements
...
String Device_Name = "Chicken Car";
String Version = "2022-06-24";
...
void loop() {
  ...
  if (btn_music_mode == LOW) {
		RF_send("Chicken Car ... <btn_music_mode>");
...
void turn_music_on_or_off() {
  ...
  if (gbl_chicken_music_on_off == 0)  {
    ...
    if (gbl_music_mode == -1) {
      ...
      myDFPlayer.volume(volume_range_array[gbl_music_volume_idx]);  
      myDFPlayer.loopFolder(PlaylistFolder_nbr[gbl_music_mode]); //1=playlist #1; 2=playlist #2; 3=playlist #3.... gbl_nbr_playlists
    }
  }
}
...
void RF_send(String RFMsgtoSend) {
		...
		RFMsgtoSend.toCharArray(text, RF_msg_len);
...
/*
	-- CC:What Version 
*/
		if (RF_Master_Control_Request_Msg == "CC:What Version") {  // send RF messages
			RF_send("Chicken Car ... " + Version);

		} else if (RF_Master_Control_Request_Msg == "radio:on+") { //int chicken_radio_on_off = 0; //0 = radio OFF, 1 = radio ON
		...
		} else if (RF_Master_Control_Request_Msg == "radio:off") {
